...
ACTION_COLORS = {
    ...
    "Activity Notice": discord.Color.orange(),
}
ACTION_ICONS = {
...
COMMAND_SYNTAX = {
    ...
    "quotaset": {
        "usage":   ".quotaset <role> <messages> <cases>",
        "example": ".quotaset @Moderator 100 15",
        "note":    "Sets the weekly message and mod case quota for a role.",
        "perms":   "Configurable via .setup perms",
    },
    "quotaremove": {
        "usage":   ".quotaremove <role>",
        "example": ".quotaremove @Moderator",
        "note":    "Removes the quota configured for a role.",
        "perms":   "Configurable via .setup perms",
    },
    "quotaview": {
        "usage":   ".quotaview [@staff]",
        "example": ".quotaview @Jane",
        "note":    "Shows a staff member's weekly quota progress.",
        "perms":   "Configurable via .setup perms",
    },
    "quotalist": {
        "usage":   ".quotalist",
        "example": ".quotalist",
        "note":    "Lists every role with a quota configured.",
        "perms":   "Configurable via .setup perms",
    },
    "quotaleaderboard": {
        "usage":   ".quotaleaderboard",
        "example": ".quotaleaderboard",
        "note":    "Shows every staff member's weekly quota progress, ranked by who's leading.",
        "perms":   "Configurable via .setup perms",
    },
    "quotareset": {
        "usage":   ".quotareset",
        "example": ".quotareset",
        "note":    "Wipes everyone's quota progress and starts a fresh tracking period. Quota targets (.quotaset) are untouched. Who can run this is configurable separately from .quotaset under .setup perms → Staff Management → Quota Reset Access.",
        "perms":   "Configurable via .setup perms (separate from .quotaset access)",
    },
    "quotacheck": {
        "usage":   ".quotacheck",
        "example": ".quotacheck",
        "note":    "Shows three embeds — who passed, who failed, and who's on approved LOA (LOA members are excluded from pass/fail). Includes buttons to reset the quota or send activity notices to everyone who failed.",
        "perms":   "Configurable via .setup perms",
    },
    "loa": {
...
def add_case(data, guild_id, action, moderator, target, reason) -> int:
    ...
    increment_quota_cases(data, guild_id, moderator.id)
    save_data(data)
...
def get_demote_remove_role_ids(guild_id: int) -> list:
    data = load_data()
    val = data.get("demote_remove_role", {}).get(str(guild_id))
    if val is None:
        return []
    if isinstance(val, list):
        return val
    return [val]  # legacy single-role value migrated to a list on read

def set_demote_remove_roles(guild_id: int, role_ids: list):
    data = load_data()
    data.setdefault("demote_remove_role", {})[str(guild_id)] = list(role_ids)
    save_data(data)

def get_stafflist_role_ids(guild_id: int) -> list:
    data = load_data()
    return data.get("stafflist_roles", {}).get(str(guild_id), [])

def set_stafflist_roles(guild_id: int, role_ids: list):
    data = load_data()
    data.setdefault("stafflist_roles", {})[str(guild_id)] = list(role_ids)
    save_data(data)
...
# ══════════════════════════════════════════════════════════════════════════════
#  STAFF QUOTA SYSTEM — weekly message + mod case quotas, configurable per role
# ══════════════════════════════════════════════════════════════════════════════

def get_guild_quotas(guild_id: int) -> dict:
    """Returns {role_id_str: {"messages": int, "cases": int}} for a guild."""
    data = load_data()
    return data.get("quotas", {}).get(str(guild_id), {})

def get_role_quota(guild_id: int, role_id: int) -> dict | None:
    return get_guild_quotas(guild_id).get(str(role_id))

def set_role_quota(guild_id: int, role_id: int, messages: int, cases: int):
    data = load_data()
    data.setdefault("quotas", {}).setdefault(str(guild_id), {})[str(role_id)] = {
        "messages": messages, "cases": cases,
    }
    save_data(data)

def remove_role_quota(guild_id: int, role_id: int) -> bool:
    data = load_data()
    guild_quotas = data.get("quotas", {}).get(str(guild_id), {})
    if str(role_id) not in guild_quotas:
        return False
    del guild_quotas[str(role_id)]
    save_data(data)
    return True

def get_applicable_quota(member: discord.Member) -> tuple:
    """Returns (role, quota_dict) for the highest-position role the member
    holds that has a quota configured, or (None, None) if none apply."""
    quotas = get_guild_quotas(member.guild.id)
    if not quotas:
        return None, None
    for role in sorted(member.roles, key=lambda r: r.position, reverse=True):
        q = quotas.get(str(role.id))
        if q:
            return role, q
    return None, None

def get_weekly_case_count(data: dict, guild_id: int, mod_id: int) -> int:
    """Counts mod cases (bans/kicks/mutes/warns/etc.) logged by mod_id in the
    current calendar week, using the same week-bucketing as message stats."""
    now = datetime.now(timezone.utc)
    week_key = now.strftime("%Y-%W")
    actions = get_mod_actions(data, guild_id, mod_id).get("actions", [])
    count = 0
    for a in actions:
        ts = a.get("timestamp")
        if not ts:
        ...
        try:
            ...
            continue
        if dt.strftime("%Y-%W") == week_key:
            count += 1
    return count
# ── Resettable quota progress counters ──────────────────────────────────────
# Unlike message_stats/mod_actions (which bucket by calendar week and reset
# themselves automatically), these counters only move forward until someone
# explicitly resets them with .quotareset / /quota reset. That's what makes a
# manual reset meaningful -- otherwise "reset the quota" has nothing to do.

def _quota_progress_bucket(data: dict, guild_id: int) -> dict:
    return data.setdefault("quota_progress", {}).setdefault(str(guild_id), {})

def increment_quota_messages(data: dict, guild_id: int, user_id: int, amount: int = 1):
    bucket = _quota_progress_bucket(data, guild_id)
    rec = bucket.setdefault(str(user_id), {"messages": 0, "cases": 0})
    rec["messages"] = rec.get("messages", 0) + amount

def increment_quota_cases(data: dict, guild_id: int, user_id: int, amount: int = 1):
    bucket = _quota_progress_bucket(data, guild_id)
    rec = bucket.setdefault(str(user_id), {"messages": 0, "cases": 0})
    rec["cases"] = rec.get("cases", 0) + amount

def get_quota_progress(data: dict, guild_id: int, user_id: int) -> dict:
    rec = _quota_progress_bucket(data, guild_id).get(str(user_id), {})
    return {"messages": rec.get("messages", 0), "cases": rec.get("cases", 0)}

def reset_guild_quota_progress(guild_id: int, moderator=None):
    """Wipes quota progress for every member in this guild and stamps the
    reset time. Does NOT touch the quota configuration itself (.quotaset)."""
    data = load_data()
    data.setdefault("quota_progress", {})[str(guild_id)] = {}
    data.setdefault("quota_reset_at", {})[str(guild_id)] = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "by_id": moderator.id if moderator else None,
        "by_tag": str(moderator) if moderator else None,
    }
    save_data(data)

def get_quota_reset_info(guild_id: int) -> dict | None:
    data = load_data()
    return data.get("quota_reset_at", {}).get(str(guild_id))

def build_quota_progress_embed(guild: discord.Guild, member: discord.Member) -> discord.Embed:
    role, quota = get_applicable_quota(member)
    embed = discord.Embed(
        title=f"📊 Weekly Quota — {member.display_name}",
        color=discord.Color.blurple(),
        timestamp=datetime.now(timezone.utc),
    )
    embed.set_thumbnail(url=member.display_avatar.url)
    if not quota:
        embed.description = "*No quota is configured for any role this member holds.*"
        return embed
    data         = load_data()
    progress     = get_quota_progress(data, guild.id, member.id)
    weekly_msgs  = progress["messages"]
    weekly_cases = progress["cases"]
    msg_target  = quota.get("messages", 0)
    case_target = quota.get("cases", 0)

    def _line(current, target, label, emoji):
        if target <= 0:
            return f"{emoji} **{label}:** {current} *(no quota set)*"
        pct  = min(100, round((current / target) * 100)) if target else 0
        icon = "✅" if current >= target else "🔸"
        return f"{emoji} **{label}:** {current} / {target}  ({pct}%)  {icon}"
    embed.add_field(name="🎖️ Quota Role", value=role.mention, inline=False)
    embed.add_field(name="Progress", value=(
        f"{_line(weekly_msgs, msg_target, 'Messages', '💬')}\n"
        f"{_line(weekly_cases, case_target, 'Mod Cases', '🛡️')}"
    ), inline=False)
    reset_info = get_quota_reset_info(guild.id)
    if reset_info and reset_info.get("timestamp"):
        reset_ts = int(datetime.fromisoformat(reset_info["timestamp"]).timestamp())
        embed.add_field(name="🔄 Tracking Since", value=f"<t:{reset_ts}:R>", inline=False)
    else:
        embed.add_field(name="🔄 Tracking Since", value="*Never reset — use `.quotareset` to start a fresh period*", inline=False)
    embed.set_footer(text="Progress persists until manually reset")
    return embed

def get_quota_members(guild: discord.Guild) -> dict:
    """Returns {member_id: (member, role, quota)} — one entry per staff
    member, using the highest-position quota-configured role they hold so
    nobody is counted twice if they have several quota roles."""
    quotas = get_guild_quotas(guild.id)
    if not quotas:
        return {}
    roles = []
    for role_id_str in quotas:
    ...
    result = {}
    for role in roles:
        q = quotas[str(role.id)]
        for member in role.members:
            if member.id not in result:
                result[member.id] = (member, role, q)
    return result

def build_quota_leaderboard_embed(guild: discord.Guild) -> discord.Embed:
    members_map = get_quota_members(guild)
    embed = discord.Embed(
        title="🏆 Staff Quota Leaderboard",
        color=discord.Color.gold(),
        timestamp=datetime.now(timezone.utc),
    )
    if not members_map:
        embed.description = "*No staff currently hold a role with a quota configured.*"
        return embed
    data = load_data()
    rows = []
    for member, role, q in members_map.values():
        progress     = get_quota_progress(data, guild.id, member.id)
        weekly_msgs  = progress["messages"]
        weekly_cases = progress["cases"]
        msg_target   = q.get("messages", 0)
        case_target  = q.get("cases", 0)
        pcts = []
        if msg_target > 0:
            pcts.append(min(1.0, weekly_msgs / msg_target))
        if case_target > 0:
            pcts.append(min(1.0, weekly_cases / case_target))
        overall = (sum(pcts) / len(pcts)) if pcts else 0.0
        rows.append({
            "member": member, "role": role,
            "msgs": weekly_msgs, "msg_target": msg_target,
            "cases": weekly_cases, "case_target": case_target,
            "overall": overall,
        })
    # Leader = highest overall completion; ties broken by raw message count then case count.
    rows.sort(key=lambda r: (r["overall"], r["msgs"], r["cases"]), reverse=True)
    medals = ["🥇", "🥈", "🥉"]
    lines  = []
    for i, r in enumerate(rows[:15]):
        rank = medals[i] if i < 3 else f"`#{i + 1}`"
        pct  = round(r["overall"] * 100)
        met  = " ✅" if r["overall"] >= 1.0 else ""
        msg_disp  = f"{r['msgs']}/{r['msg_target']}" if r["msg_target"] > 0 else f"{r['msgs']} *(no quota)*"
        case_disp = f"{r['cases']}/{r['case_target']}" if r["case_target"] > 0 else f"{r['cases']} *(no quota)*"
        lines.append(
            f"{rank} {r['member'].mention} — **{pct}%**{met}\n"
            f"　　💬 {msg_disp}  •  🛡️ {case_disp}  •  {r['role'].mention}"
        )
    embed.description = "\n".join(lines)
    if rows:
        leader = rows[0]["member"]
        embed.add_field(name="👑 Currently Leading", value=leader.mention, inline=False)
    footer = "Progress persists until manually reset"
    if len(rows) > 15:
...
PERM_CATEGORIES = {
    ...
    "🧑‍💼  Staff Management": {
        ...
        "commands": ["infraction", "infractions", "removeinfraction", "promote", "demote",
                     ...
                     "quotaset", "quotaremove", "quotaview", "quotalist", "quotaleaderboard",
                     "quotareset", "quotacheck"],
    },
    "🌴  LOA Channel": {
        "emoji": "🌴",
    ...
    "🗑️  Demotion Role Removed": {
        "emoji": "🗑️",
        "description": "The role(s) automatically removed once a demotion request is approved",
        "commands": [],
    },
    "🧑‍💼  Staff List Roles": {
        "emoji": "🧑‍💼",
        "description": "Choose exactly which roles show up in .stafflist",
        "commands": [],
    },
    "📣  Mass Echo": {
        "emoji": "📣",
...
COMMAND_EMOJIS = {
    ...
    "stafflist": "🧑‍💼", "massecho": "📣",
    "quotaset": "📊", "quotaremove": "🗑️", "quotaview": "📈", "quotalist": "📋", "quotaleaderboard": "🏆",
    "quotareset": "🔄", "quotacheck": "🧾",
}
...
class CategorySelectDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if cat_name == "🗑️  Demotion Role Removed":
            ...
            await interaction.response.edit_message(embed=embed, view=view)
            return
        if cat_name == "🧑‍💼  Staff List Roles":
            view  = StaffListRoleConfigView(interaction.guild, self.guild_cmd_roles, self.invoker_id)
            embed = view.build_embed()
            await interaction.response.edit_message(embed=embed, view=view)
            return
...
class DemoteRoleConfigSelect(discord.ui.RoleSelect):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
        ...
        self.invoker_id      = invoker_id
        super().__init__(
            placeholder="Choose role(s) removed on demotion approval...",
            min_values=0, max_values=25,
        )

    async def callback(self, interaction: discord.Interaction):
        roles = list(self.values)
        set_demote_remove_roles(self.guild.id, [r.id for r in roles])
        view  = DemoteRoleConfigView(self.guild, self.guild_cmd_roles, self.invoker_id)
        embed = view.build_embed(updated_to=roles)
        await interaction.response.edit_message(embed=embed, view=view)

class DemoteRoleClearButton(discord.ui.Button):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
    ...
    async def callback(self, interaction: discord.Interaction):
        set_demote_remove_roles(self.guild.id, [])
        view  = DemoteRoleConfigView(self.guild, self.guild_cmd_roles, self.invoker_id)
        embed = view.build_embed(updated_to=[])
        await interaction.response.edit_message(embed=embed, view=view)
...
class DemoteRoleConfigView(discord.ui.View):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
        ...
        self.add_item(DemoteRoleClearButton(guild, guild_cmd_roles, invoker_id))
        self.add_item(BackToCategoryButton(guild_cmd_roles, invoker_id))

    def build_embed(self, updated_to: list = None) -> discord.Embed:
        role_ids = get_demote_remove_role_ids(self.guild.id)
        roles = [self.guild.get_role(rid) for rid in role_ids]
        roles = [r for r in roles if r]
        embed = discord.Embed(
            title="🗑️  Demotion Role(s) Removed",
            description=(
                "Choose the role(s) the bot should automatically **remove** from a "
                "staff member whenever a demotion request is approved via "
                "`.demotereq` / `/demoterequest`.\n\n"
                "**Selecting again replaces the current list — use Clear to remove all.**"
            ),
            color=discord.Color.blurple(),
            timestamp=datetime.now(timezone.utc),
        )
        if updated_to is not None:
            value = ", ".join(r.mention for r in updated_to) if updated_to else "*None — no roles will be auto-removed*"
            embed.add_field(name="✅ Updated", value=value, inline=False)
        elif roles:
            embed.add_field(name="Current Roles", value=", ".join(r.mention for r in roles), inline=False)
        else:
            embed.add_field(name="Current Roles", value="*Not set — no roles will be auto-removed*", inline=False)
        embed.set_footer(text="Admins and the bot owner can always use any command.")
        return embed
...
class StaffListRoleConfigSelect(discord.ui.RoleSelect):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
        self.guild           = guild
        self.guild_cmd_roles = guild_cmd_roles
        self.invoker_id      = invoker_id
        super().__init__(
            placeholder="Choose role(s) shown in .stafflist...",
            min_values=0, max_values=25,
        )

    async def callback(self, interaction: discord.Interaction):
        roles = list(self.values)
        set_stafflist_roles(self.guild.id, [r.id for r in roles])
        view  = StaffListRoleConfigView(self.guild, self.guild_cmd_roles, self.invoker_id)
        embed = view.build_embed(updated_to=roles)
        await interaction.response.edit_message(embed=embed, view=view)

class StaffListRoleClearButton(discord.ui.Button):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
        super().__init__(label="Clear (auto-detect again)", style=discord.ButtonStyle.danger, emoji="🗑️")
        self.guild           = guild
        self.guild_cmd_roles = guild_cmd_roles
        self.invoker_id      = invoker_id

    async def callback(self, interaction: discord.Interaction):
        set_stafflist_roles(self.guild.id, [])
        view  = StaffListRoleConfigView(self.guild, self.guild_cmd_roles, self.invoker_id)
        embed = view.build_embed(updated_to=[])
        await interaction.response.edit_message(embed=embed, view=view)

class StaffListRoleConfigView(discord.ui.View):
    def __init__(self, guild: discord.Guild, guild_cmd_roles: dict, invoker_id: int):
        super().__init__(timeout=300)
        self.guild           = guild
        self.guild_cmd_roles = guild_cmd_roles
        self.invoker_id      = invoker_id
        self.add_item(StaffListRoleConfigSelect(guild, guild_cmd_roles, invoker_id))
        self.add_item(StaffListRoleClearButton(guild, guild_cmd_roles, invoker_id))
        self.add_item(BackToCategoryButton(guild_cmd_roles, invoker_id))

    def build_embed(self, updated_to: list = None) -> discord.Embed:
        role_ids = get_stafflist_role_ids(self.guild.id)
        roles = [self.guild.get_role(rid) for rid in role_ids]
        roles = [r for r in roles if r]
        embed = discord.Embed(
            title="🧑‍💼  Staff List Roles",
            description=(
                "Choose exactly which roles should appear in `.stafflist` / `/stafflist`.\n\n"
                "**Selecting again replaces the current list — use Clear to fall back to "
        ...
        if updated_to is not None:
            value = ", ".join(r.mention for r in updated_to) if updated_to else "*None — falling back to automatic keyword detection*"
            embed.add_field(name="✅ Updated", value=value, inline=False)
        elif roles:
            embed.add_field(name="Current Roles", value=", ".join(r.mention for r in roles), inline=False)
        else:
            embed.add_field(name="Current Roles", value="*Not set — falls back to automatic keyword detection*", inline=False)
        embed.set_footer(text="Admins and the bot owner can always use any command.")
        return embed
...
async def help_cmd(ctx):
    ...
    embed.add_field(name="🧑‍💼  Staff Management — Configurable via .setup perms", value=(
        ...
        "`/stafflist` `/stafflist` — Same as above (slash version)\n"
        "`quotaset` `.quotaset <role> <messages> <cases>` — Set a role's weekly quota\n"
        "`/quota set` `/quota set` — Same as above (slash version)\n"
        "`quotaremove` `.quotaremove <role>` — Remove a role's quota\n"
        "`/quota remove` `/quota remove` — Same as above (slash version)\n"
        "`quotaview` `.quotaview [@staff]` — Check weekly quota progress\n"
        "`/quota view` `/quota view` — Same as above (slash version)\n"
        "`quotalist` `.quotalist` — List all configured role quotas\n"
        "`/quota list` `/quota list` — Same as above (slash version)\n"
        "`quotaleaderboard` `.quotaleaderboard` — See who's leading on quota this week\n"
        "`/quota leaderboard` `/quota leaderboard` — Same as above (slash version)\n"
        "`quotareset` `.quotareset` — Wipe everyone's quota progress and start fresh\n"
        "`/quota reset` `/quota reset` — Same as above (slash version)\n"
        "`quotacheck` `.quotacheck` — Show who passed/failed/on LOA, with Reset & Send Notices buttons\n"
        "`/quota check` `/quota check` — Same as above (slash version)"), inline=False)
...
def build_demotion_request_embed(rid: int, requester, target, role: discord.Role, reason: str, status: str = "pending", remove_role=None) -> discord.Embed:
    color = discord.Color.orange() if status == "pending" else (discord.Color.green() if status == "approved" else discord.Color.red())
    icon  = "🟡" if status == "pending" else ("✅" if status == "approved" else "❌")
    embed = discord.Embed(
    ...
    embed.add_field(name="🛡️ Requested by", value=f"{requester}", inline=True)
    embed.add_field(name="🎖️ New Role",     value=role.mention if isinstance(role, discord.Role) else str(role), inline=True)
    remove_roles = remove_role if isinstance(remove_role, (list, tuple)) else ([remove_role] if remove_role else [])
    if remove_roles:
        label = "🗑️ Role(s) Removed on Approval" if len(remove_roles) > 1 else "🗑️ Role Removed on Approval"
        embed.add_field(name=label, value=", ".join(r.mention for r in remove_roles), inline=True)
    embed.add_field(name="📄 Reason",       value=reason or "No reason provided", inline=False)
...
class DemotionRequestActionView(discord.ui.View):
    ...
    async def approve_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if err:
            return await interaction.followup.send(embed=err, ephemeral=True)
        # Remove the configured "roles removed on demotion", if any and if the target has them.
        remove_role_ids = get_demote_remove_role_ids(guild.id)
        removed_roles   = []
        forbidden_roles = []
        to_remove = []
        ...
        if to_remove:
            try:
                await target.remove_roles(*to_remove, reason=f"Demotion #{self.rid} approved by {interaction.user}")
                removed_roles = to_remove
            except discord.Forbidden:
                forbidden_roles = to_remove
                await interaction.followup.send(
                    embed=warn_embed(f"Demotion applied, but I don't have permission to remove: {', '.join(r.mention for r in forbidden_roles)}."),
                    ephemeral=True)
        _mark_request_status(load_data(), "demotion_requests", self.guild_id, self.rid, "approved", interaction.user)
        self._actioned = True
        self._disable_all()
        try:
            req_embed = build_demotion_request_embed(self.rid, req["requester_tag"], target, role, req["reason"], status="approved", remove_role=removed_roles)
            await interaction.message.edit(embed=req_embed, view=self)
        except Exception:
            pass
        note = f"Demotion Request #{self.rid} approved — **{target}** is now **{role.name}**."
        if removed_roles:
            note += f" **{', '.join(r.name for r in removed_roles)}** {'was' if len(removed_roles) == 1 else 'were'} removed."
...
async def demotereq_cmd(ctx, member: discord.Member = None, role: discord.Role = None, *, reason="No reason provided"):
    ...
    remove_roles = [r for r in (ctx.guild.get_role(rid2) for rid2 in get_demote_remove_role_ids(ctx.guild.id)) if r]
    embed = build_demotion_request_embed(rid, ctx.author, member, role, reason, remove_role=remove_roles)
...
async def demotereq_slash(interaction: discord.Interaction, member: discord.Member, role: discord.Role, reason: str = "No reason provided"):
    ...
    remove_roles = [r for r in (interaction.guild.get_role(rid2) for rid2 in get_demote_remove_role_ids(interaction.guild.id)) if r]
    embed = build_demotion_request_embed(rid, interaction.user, member, role, reason, remove_role=remove_roles)
...
def build_staff_list_embed(guild: discord.Guild) -> discord.Embed:
    configured_ids = get_stafflist_role_ids(guild.id)
    if configured_ids:
        staff_roles = [guild.get_role(rid) for rid in configured_ids]
    ...
    embed = discord.Embed(title=f"🧑‍💼 Staff List — {guild.name}", color=discord.Color.blurple(),
                          timestamp=datetime.now(timezone.utc))
    if not staff_roles:
        embed.description = ("No staff roles are currently set." if configured_ids
                              else "No staff roles matched the configured keywords.")
        return embed
    seen_ids = set()
    total = 0
...
# ══════════════════════════════════════════════════════════════════════════════
#  STAFF QUOTA SYSTEM — weekly message + mod case quotas, per role
# ══════════════════════════════════════════════════════════════════════════════

def build_quota_list_embed(guild: discord.Guild) -> discord.Embed:
    quotas = get_guild_quotas(guild.id)
    embed = discord.Embed(
        title="📊 Configured Staff Quotas",
        color=discord.Color.blurple(),
        timestamp=datetime.now(timezone.utc),
    )
    if not quotas:
        embed.description = "*No quotas have been configured yet. Use `.quotaset` / `/quota set` to add one.*"
        return embed
    lines = []
    for role_id_str, q in quotas.items():
        role = guild.get_role(int(role_id_str))
        role_display = role.mention if role else f"`Deleted role ({role_id_str})`"
        lines.append(f"{role_display} — 💬 **{q.get('messages', 0)}** messages, 🛡️ **{q.get('cases', 0)}** cases /week")
    embed.description = "\n".join(lines)
    embed.set_footer(text="Targets are weekly, but progress only resets via .quotareset")
    return embed

def compute_quota_overview(guild: discord.Guild) -> dict:
    """Splits every quota-tracked staff member into passed / failed / on_loa.
    A member counts as 'on LOA' (and is excluded from pass/fail) only if they
    have a currently-approved LOA record. Passing requires meeting BOTH the
    message and case targets (targets of 0 count as automatically met)."""
    data         = load_data()
    members_map  = get_quota_members(guild)
    active_loas  = [l for l in get_active_loas(data, guild.id) if l["status"] == "approved"]
    loa_user_ids = {l["user_id"] for l in active_loas}
    passed, failed, on_loa = [], [], []
    for member_id, (member, role, q) in members_map.items():
        if member_id in loa_user_ids:
            on_loa.append({"member": member, "role": role})
            continue
        progress    = get_quota_progress(data, guild.id, member_id)
        msg_target  = q.get("messages", 0)
        case_target = q.get("cases", 0)
        msg_ok      = progress["messages"] >= msg_target if msg_target > 0 else True
        case_ok     = progress["cases"]    >= case_target if case_target > 0 else True
        entry = {
            "member": member, "role": role,
            "messages": progress["messages"], "msg_target": msg_target,
            "cases": progress["cases"], "case_target": case_target,
        }
        (passed if (msg_ok and case_ok) else failed).append(entry)
    return {"passed": passed, "failed": failed, "on_loa": on_loa}

def _quota_entry_line(e: dict) -> str:
    m_disp = f"{e['messages']}/{e['msg_target']}" if e["msg_target"] > 0 else f"{e['messages']} *(no target)*"
    c_disp = f"{e['cases']}/{e['case_target']}" if e["case_target"] > 0 else f"{e['cases']} *(no target)*"
    return f"{e['member'].mention} — {e['role'].mention}\n　　💬 {m_disp}  •  🛡️ {c_disp}"

def _quota_embed_chunks(title: str, color, entries, empty_text: str, footer: str) -> list:
    """Builds one or more embeds (splitting on Discord's field/description
    limits) for a list of quota entries."""
    if not entries:
        ...
        return [embed]
    embeds = []
    lines  = [_quota_entry_line(e) for e in entries]
    chunk  = []
    length = 0
    for line in lines:
        if length + len(line) + 2 > 3800 or len(chunk) >= 20:
            embeds.append(chunk)
            chunk, length = [], 0
        chunk.append(line)
        length += len(line) + 2
    if chunk:
        embeds.append(chunk)
    result = []
    for i, part in enumerate(embeds):
        part_title = title if len(embeds) == 1 else f"{title}  (part {i + 1}/{len(embeds)})"
        embed = discord.Embed(title=part_title, description="\n\n".join(part), color=color,
                              timestamp=datetime.now(timezone.utc))
        embed.set_footer(text=f"{footer}  •  {len(entries)} total")
        result.append(embed)
    return result

def build_quota_check_embeds(guild: discord.Guild) -> list:
    overview = compute_quota_overview(guild)
    reset_info = get_quota_reset_info(guild.id)
    if reset_info and reset_info.get("timestamp"):
        reset_ts = int(datetime.fromisoformat(reset_info["timestamp"]).timestamp())
        footer   = f"Tracking since <t:{reset_ts}:R>"
    else:
        footer = "Never reset — use the button below or .quotareset to start fresh"
    if not overview["passed"] and not overview["failed"] and not overview["on_loa"]:
        return [discord.Embed(
            title="📊 Quota Check",
            description="*No staff currently hold a role with a quota configured.*",
            color=discord.Color.blurple(),
        )]
    embeds  = []
    embeds += _quota_embed_chunks("✅ Passed Quota", discord.Color.green(), overview["passed"],
                                  "*Nobody has met quota yet.*", footer)
    embeds += _quota_embed_chunks("❌ Failed Quota", discord.Color.red(), overview["failed"],
                                  "*Nobody is currently failing quota.*", footer)
    embeds += _quota_embed_chunks("🌴 On LOA (excluded from pass/fail)", discord.Color.dark_gray(), overview["on_loa"],
                                  "*Nobody with a quota role is currently on approved LOA.*", footer)
    return embeds

class QuotaCheckView(discord.ui.View):
    """Attached to `.quotacheck` / `/quota check`. Both buttons are gated by
    the same permission as `.quotareset` and `.infraction` respectively, so
    setting those roles via `.setup perms` controls who can use them here too."""

    def __init__(self, invoker_id: int):
        super().__init__(timeout=300)
        self.invoker_id = invoker_id

    async def _refresh_embeds(self, interaction: discord.Interaction):
        embeds = build_quota_check_embeds(interaction.guild)
        await interaction.edit_original_response(embeds=embeds, view=self)

    @discord.ui.button(label="🔄 Reset Quota", style=discord.ButtonStyle.secondary)
    async def reset_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not member_has_cmd_perm(interaction.user, "quotareset"):
            return await interaction.response.send_message(
                embed=error_embed("You don't have permission to reset the quota."), ephemeral=True)
        reset_guild_quota_progress(interaction.guild.id, moderator=interaction.user)
        await interaction.response.defer()
        await self._refresh_embeds(interaction)
        await interaction.followup.send(
            embed=success_embed(f"Quota progress reset by {interaction.user.mention}. Everyone starts fresh."))

    @discord.ui.button(label="📨 Send Activity Notices", style=discord.ButtonStyle.danger)
    async def notify_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not member_has_cmd_perm(interaction.user, "infraction"):
            return await interaction.response.send_message(
                embed=error_embed("You don't have permission to issue activity notices."), ephemeral=True)
        await interaction.response.defer()
        overview = compute_quota_overview(interaction.guild)
        failed   = overview["failed"]
        if not failed:
            return await interaction.followup.send(
                embed=info_embed("Nobody is currently failing quota — no notices sent."))
        data   = load_data()
        guild  = interaction.guild
        sent   = 0
        failed_dm = []
        for e in failed:
            member = e["member"]
            reason = (
                f"Failed weekly activity quota — "
                f"💬 {e['messages']}/{e['msg_target'] if e['msg_target'] > 0 else '—'} messages, "
                f"🛡️ {e['cases']}/{e['case_target'] if e['case_target'] > 0 else '—'} mod cases."
            )
            num = add_staff_infraction(data, guild.id, member.id, interaction.user, reason, severity="Activity Notice")
            cid = add_case(data, guild.id, "Activity Notice", interaction.user, member, reason)
            dm_ok = await dm_member(member, "Activity Notice", guild.name, reason, cid,
                                    moderator=interaction.user,
                                    extra=f"This is infraction **#{num}** on your staff record.")
            if not dm_ok:
                failed_dm.append(member.mention)
            await post_mod_log(guild, cid, "Activity Notice", interaction.user, member, reason,
                               extra=f"Issued from `.quotacheck` — staff infraction #{num}")
            sent += 1
        summary = f"Sent **{sent}** activity notice(s) to members who failed quota."
        if failed_dm:
            summary += f"\n⚠️ Could not DM: {', '.join(failed_dm)} (still logged as case + infraction)."
        await interaction.followup.send(embed=success_embed(summary))

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        # Anyone who could run .quotacheck in the first place can view/refresh;
        # the buttons themselves re-check the stricter individual permissions.
        return True
...
@bot.command(name="quotareset")
@require_cmd_role("quotareset")
async def quotareset_cmd(ctx):
    reset_guild_quota_progress(ctx.guild.id, moderator=ctx.author)
    await ctx.send(embed=success_embed(
        f"Quota progress reset by {ctx.author.mention}. Everyone's messages and mod cases now start from 0."))

@quotareset_cmd.error
async def quotareset_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to reset the quota."))

@bot.command(name="quotacheck", aliases=["quotastatus", "quotaoverview"])
@require_cmd_role("quotacheck")
async def quotacheck_cmd(ctx):
    embeds = build_quota_check_embeds(ctx.guild)
    view = QuotaCheckView(ctx.author.id)
    await ctx.send(embeds=embeds, view=view)

@quotacheck_cmd.error
async def quotacheck_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to view the quota check."))

@bot.command(name="quotaset")
@require_cmd_role("quotaset")
async def quotaset_cmd(ctx, role: discord.Role = None, messages: int = None, cases: int = None):
    if role is None or messages is None or cases is None:
        return await ctx.send(embed=syntax_embed("quotaset"))
    if messages < 0 or cases < 0:
        return await ctx.send(embed=error_embed("Quota values can't be negative."))
    set_role_quota(ctx.guild.id, role.id, messages, cases)
    await ctx.send(embed=success_embed(
        f"Weekly quota for {role.mention} set to **{messages}** messages and **{cases}** mod cases."))

@quotaset_cmd.error
async def quotaset_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to set quotas."))
    elif isinstance(error, (commands.RoleNotFound, commands.BadArgument)):
        await ctx.send(embed=error_embed("Usage: `.quotaset @role <messages> <cases>`"))

@bot.command(name="quotaremove")
@require_cmd_role("quotaremove")
async def quotaremove_cmd(ctx, role: discord.Role = None):
    if role is None:
        return await ctx.send(embed=syntax_embed("quotaremove"))
    if remove_role_quota(ctx.guild.id, role.id):
        await ctx.send(embed=success_embed(f"Quota removed for {role.mention}."))
    else:
        await ctx.send(embed=warn_embed(f"{role.mention} doesn't have a quota configured."))

@quotaremove_cmd.error
async def quotaremove_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to remove quotas."))
    elif isinstance(error, (commands.RoleNotFound, commands.BadArgument)):
        await ctx.send(embed=error_embed("Usage: `.quotaremove @role`"))

@bot.command(name="quotaview")
@require_cmd_role("quotaview")
async def quotaview_cmd(ctx, member: discord.Member = None):
    target = member or ctx.author
    await ctx.send(embed=build_quota_progress_embed(ctx.guild, target))

@quotaview_cmd.error
async def quotaview_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to view quotas."))
    elif isinstance(error, commands.BadArgument):
        await ctx.send(embed=error_embed("Usage: `.quotaview [@staff]`"))

@bot.command(name="quotalist")
@require_cmd_role("quotalist")
async def quotalist_cmd(ctx):
    await ctx.send(embed=build_quota_list_embed(ctx.guild))

@quotalist_cmd.error
async def quotalist_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to view quotas."))

@bot.command(name="quotaleaderboard", aliases=["quotalb", "quotatop"])
@require_cmd_role("quotaleaderboard")
async def quotaleaderboard_cmd(ctx):
    await ctx.send(embed=build_quota_leaderboard_embed(ctx.guild))

@quotaleaderboard_cmd.error
async def quotaleaderboard_cmd_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send(embed=error_embed("You don't have permission to view the quota leaderboard."))
quota_group = app_commands.Group(name="quota", description="Staff quota management")

@quota_group.command(name="set", description="[Staff Mgmt] Set a role's weekly message and mod case quota")
@app_commands.describe(role="Staff role", messages="Weekly message quota", cases="Weekly mod case quota")
@slash_cmd_role("quotaset")
async def quota_set_slash(interaction: discord.Interaction, role: discord.Role, messages: int, cases: int):
    if messages < 0 or cases < 0:
        return await interaction.response.send_message(embed=error_embed("Quota values can't be negative."), ephemeral=True)
    set_role_quota(interaction.guild.id, role.id, messages, cases)
    await interaction.response.send_message(embed=success_embed(
        f"Weekly quota for {role.mention} set to **{messages}** messages and **{cases}** mod cases."))

@quota_set_slash.error
async def quota_set_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="remove", description="[Staff Mgmt] Remove a role's configured quota")
@app_commands.describe(role="Staff role")
@slash_cmd_role("quotaremove")
async def quota_remove_slash(interaction: discord.Interaction, role: discord.Role):
    if remove_role_quota(interaction.guild.id, role.id):
        await interaction.response.send_message(embed=success_embed(f"Quota removed for {role.mention}."))
    else:
        await interaction.response.send_message(embed=warn_embed(f"{role.mention} doesn't have a quota configured."), ephemeral=True)

@quota_remove_slash.error
async def quota_remove_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="view", description="Check a staff member's weekly quota progress")
@app_commands.describe(member="Staff member (optional — defaults to you)")
@slash_cmd_role("quotaview")
async def quota_view_slash(interaction: discord.Interaction, member: discord.Member = None):
    target = member or interaction.user
    await interaction.response.send_message(embed=build_quota_progress_embed(interaction.guild, target))

@quota_view_slash.error
async def quota_view_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="list", description="[Staff Mgmt] List every role with a quota configured")
@slash_cmd_role("quotalist")
async def quota_list_slash(interaction: discord.Interaction):
    await interaction.response.send_message(embed=build_quota_list_embed(interaction.guild))

@quota_list_slash.error
async def quota_list_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="leaderboard", description="See which staff member is leading on quota this week")
@slash_cmd_role("quotaleaderboard")
async def quota_leaderboard_slash(interaction: discord.Interaction):
    await interaction.response.send_message(embed=build_quota_leaderboard_embed(interaction.guild))

@quota_leaderboard_slash.error
async def quota_leaderboard_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="reset", description="[Staff Mgmt] Wipe everyone's quota progress and start fresh")
@slash_cmd_role("quotareset")
async def quota_reset_slash(interaction: discord.Interaction):
    reset_guild_quota_progress(interaction.guild.id, moderator=interaction.user)
    await interaction.response.send_message(embed=success_embed(
        f"Quota progress reset by {interaction.user.mention}. Everyone's messages and mod cases now start from 0."))

@quota_reset_slash.error
async def quota_reset_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)

@quota_group.command(name="check", description="[Staff Mgmt] Show who passed/failed quota, with reset & notify buttons")
@slash_cmd_role("quotacheck")
async def quota_check_slash(interaction: discord.Interaction):
    embeds = build_quota_check_embeds(interaction.guild)
    view = QuotaCheckView(interaction.user.id)
    await interaction.response.send_message(embeds=embeds, view=view)

@quota_check_slash.error
async def quota_check_slash_err(interaction, error):
    if isinstance(error, app_commands.CheckFailure):
        await slash_silent_fail(interaction)
bot.tree.add_command(quota_group)
# ══════════════════════════════════════════════════════════════════════════════
